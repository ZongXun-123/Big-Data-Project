...
# read df
df_topkey = pd.read_csv('app_top_keyword/dataset/ltn_news_hw3_topkey_with_category_via_token_pos.csv', sep=',')

# prepare data
data={}
for idx, row in df_topkey.iterrows():
    data[row['category']] = ast.literal_eval(row['top_keys'])
...
def get_category_topword(cate, topk=10):
    topk = int(topk)
    topk = min(topk, len(data[cate]))
    wf_pairs = data[cate][:topk] 
