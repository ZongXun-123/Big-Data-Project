...
def load_df_data_v1():
    ...
    df = pd.read_csv('app_user_keyword_sentiment/dataset/news_ltn_dataset_processed.csv',sep='|')
...
def filter_df_via_content(query_keywords, cond, cate, weeks):
    ...
    if (cond == 'and'):
        ...
        condition = condition & df.content.apply(lambda text: isinstance(text, str) and all(qk in text for qk in query_keywords))
    elif (cond == 'or'):
