...
def load_data_correlation():
    ...
    global df
    df = pd.read_csv(
        'app_correlation_analysis/dataset/news_ltn_dataset_processed.csv', sep='|')
...
def get_keyword_occurrence_time_series(query_keywords, cond='or', weeks=12):
    ...
    if (cond == 'and'):
        ...
        condition = period_condition & df.content.apply(lambda text: all((qk in str(text)) for qk in query_keywords)) #寫法:all()
    else:
        # query keywords condition使用者輸入關鍵字條件
        condition = period_condition & df.content.apply(lambda text: any((qk in str(text)) for qk in query_keywords)) #寫法:any()
